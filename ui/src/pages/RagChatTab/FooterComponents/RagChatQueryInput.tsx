...
import FileUploader from "pages/RagChatTab/FooterComponents/FileUploader.tsx";
...
const RagChatQueryInput = ({
  ...
  const handleChat = (userInput: string) => {
    if (userInput.trim().length <= 0 && pdfText.trim().length <= 0) {
      return;
    }
    if (userInput.length > 0 || pdfText.length > 0) {
      if (sessionId) {
        let re = /"(\n)|(\t)|(\r)"/g;
        const text = pdfText.replace(re, "");
        re = /"\s{2,}"/g;
        setPdfText(text.replace(re, ""));
        userInput = pdfText.length > 0 ? pdfText + userInput : userInput;
        chatMutation.mutate({
          query: userInput,
  ...
  const handlePdfText = (text: string) => {
    console.log("Extracted PDF text at parent: ", text);
    setPdfText(text);
  };
  ...
  return (
    <div>
      <Flex vertical align="center" gap={10}>
        ...
        <FileUploader takePdfText={handlePdfText} />
      </Flex>
    </div>
  );
};
